...
- `models/user.ts` contains the data-container and schema for `users`
  collection.
- `lib/config.ts` contains the initialization of Tigris client.

**Schema**
Scaffolded project provides following (`user.ts`) schema. You can extend the schema by modifying.
...
`src/lib/config.ts` initializes the client as follows

```
new Tigris({
    serverUrl: "localhost:8081",
    insecureChannel: true, // plaintext communication
}
```

This will create tigris client and point it to server running on localhost.
To use the preview cloud on Tigris platform you can modify initialization as follows
...
- It creates a db named `hello_db`
- registers collection schema `users` on the db.
- performs CRUD operations on `users`.
- performs interactive transaction.
- deletes the users from collection.
