...
func main() {
    ...
    _ = client.DropDatabase(ctx, "hello_db")
}
```
...
Now ./myproject1 binary should be present in the current directory.
...
now execute compiled binary which performs API call and outpus the results on the console:
