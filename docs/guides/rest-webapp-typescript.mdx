...
Now let's get started with Tigris, clone the application code repository:
...
You must add `TIGRIS_URI` and `TIGRIS_DB_BRANCH` variables to your environment file. Use `api.preview.tigrisdata.cloud` for `TIGRIS_URI` and `TIGRIS_DB_BRANCH`, use **main** for a production app and **develop** or another branch name for development.
...
Run the following commands to create two user records:
...
Or, search for products named "avocado" with a `price` less than `50`:
