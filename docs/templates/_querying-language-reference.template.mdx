...
Results are order by (TODO: what are they order by, by default?) by default. However, you can specify the field the results are to be sorted on and the order in which they are sorted.
